...
namespace MyTools.Utils
{
    public class SubstrTests
    {
        ...
        [Theory,
         InlineData("abc", "a", "c", "b"), InlineData("abc", "d", "e", ""), InlineData("abc", "", "", "abc"),
        ...
        [Theory,
         InlineData("abc123xyz", @"\d+", "xyz"), InlineData("123456", @"\s+", "")]
        public void AfterRegexShouldBeRight(string text, string exp, string result)
        {
            var after = new Substr(text).After(new Regex(exp));
            Assert.Equal(result, after);
        }

        [Theory,
         InlineData("abc123xyz", @"\d+", "abc"), InlineData("123456", @"\s+", "")]
        public void BeforeRegexShouldBeRight(string text, string exp, string result)
        {
            var before = new Substr(text).Before(new Regex(exp));
            Assert.Equal(result, before);
        }
        ...
        {
            var after = new Substr(text).Between(new Regex(exp1), new Regex(exp2));
            Assert.Equal(result, after);
        }
        ...
        public void AfterWithShouldBeRight(string text, string value, string result,
            StringComparison comparisonType = StringComparison.CurrentCulture)
        {
            var after = new Substr(text).AfterWith(value, comparisonType);
            Assert.Equal(result, after);
        }
        ...
        public void BeforeWithShouldBeRight(string text, string value, string result,
            StringComparison comparisonType = StringComparison.CurrentCulture)
        {
            var before = new Substr(text).BeforeWith(value, comparisonType);
            Assert.Equal(result, before);
        }
        ...
        public void BetweenWithShouldBeRight(string text, string a, string b, string result,
            StringComparison comparisonType = StringComparison.CurrentCulture)
        {
            var after = new Substr(text).BetweenWith(a, b, comparisonType);
            Assert.Equal(result, after);
        }
        ...
        {
            var before = new Substr(text).AfterWith(values);
            Assert.Equal(result, before);
        }
        ...
        {
            var before = new Substr(text).BeforeWith(values);
            Assert.Equal(result, before);
        }
        ...
        {
            var after = new Substr(text).AfterWith(new Regex(exp));
            Assert.Equal(result, after);
        }
        ...
        {
            var before = new Substr(text).BeforeWith(new Regex(exp));
            Assert.Equal(result, before);
        }
        ...
        {
            var after = new Substr(text).BetweenWith(new Regex(exp1), new Regex(exp2));
            Assert.Equal(result, after);
        }
    }
}